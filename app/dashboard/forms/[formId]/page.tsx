...
import { parseFormContent, getFormTitle, getFormFieldCount, formatDate } from "@/features/forms/utils/formUtils";
import { Edit2, ExternalLink, FileText, Users, Calendar, Globe, Lock, ArrowLeft } from "lucide-react";
...
export default async function FormDetailPage({
  ...
  const form = await getFormById(id, true);
  ...
  const createdAt = formatDate(form.createdAt);

  // @ts-expect-error FormSubmissions included via getFormById
  const recentSubmissions = form.FormSubmissions ?? [];
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className="border border-gray-200 dark:border-neutral-800">
          ...
          <CardContent>
            <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{form.submissions}</p>
        ...
        <Card className="border border-gray-200 dark:border-neutral-800">
          ...
          <CardContent>
            <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{fieldCount}</p>
          ...
          {recentSubmissions.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                ...
                {!form.published && (
                  <span>
                    <Link href={`/dashboard/forms/edit/${form.id}`} className="text-blue-600 underline">
                      Publish your form
                    </Link>{" "}
            ...
            <div className="space-y-3">
              {recentSubmissions.map((sub: { id: number; createdAt: Date }, idx: number) => (
                <div
                  key={sub.id}
                ...
                >
                  ...
                  <span className="text-xs text-gray-400">
                    {formatDate(sub.createdAt, { day: "numeric", month: "short", year: "numeric" })}
                  </span>
                </div>
              ))}
            </div>
