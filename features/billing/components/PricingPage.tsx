    ...
    <div className="py-16 px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl mx-auto items-stretch">
        {pricingPlan.map((plan: PricingPlan) => {
          ...
          return (
            <Card key={plan.level} className={`relative flex flex-col transition-all duration-200 h-full ${isPro ? "border-2 border-blue-500 shadow-2xl shadow-blue-100 dark:shadow-blue-950 bg-white dark:bg-neutral-900" : isEnterprise ? "border border-neutral-800 bg-neutral-900 dark:bg-neutral-900 text-white" : "border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900"}`}>
              {isPro && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                  <Badge className="bg-blue-600 text-white border-0 px-3 py-1 text-xs font-semibold shadow">🔥 Most Popular</Badge>
                </div>
              )}
              <CardHeader className="pt-8 pb-4">
                <CardTitle className={`text-xl font-bold ${isEnterprise ? "text-white" : "text-gray-900 dark:text-gray-100"}`}>{plan.level}</CardTitle>
                <div className="mt-2">
              ...
              <CardContent className="flex-1 pb-4">
                <ul className="space-y-3">
                  {plan.services.map((item) => (
                    <li key={item} className={`flex items-center gap-2.5 text-sm ${isEnterprise ? "text-neutral-300" : "text-gray-600 dark:text-gray-400"}`}>
              ...
              <CardFooter className="pt-2">
                <Button className={`w-full cursor-pointer font-semibold transition-all ${isPro ? "bg-blue-600 hover:bg-blue-700 text-white" : isEnterprise ? "bg-white text-black hover:bg-gray-100" : "border border-gray-300 dark:border-neutral-600 bg-transparent hover:bg-gray-50 dark:hover:bg-neutral-800 text-gray-900 dark:text-gray-100"}`} onClick={() => handlePlanClick(plan.level, plan.price)}>
