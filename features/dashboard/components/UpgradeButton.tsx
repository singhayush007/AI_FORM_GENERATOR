...
import { getUserSubscription } from "@/features/billing/actions/userSubscription";
...
const UpgradeButton: React.FC<Props> = async ({ userId }) => {
  ...
  return (
    <div className="m-3">
      {isSubscribed ? (
        <span className="text-sm">You have a subscription plan — you are eligible to create more forms.</span>
      ) : (
        <>
          <Progress value={percentage} />
          <p>
            {formsGenerated} out of {MAX_FREE_FORM} forms generated.{" "}
            <Link href="/dashboard/upgrade" className="text-blue-600 underline">Upgrade</Link>{" "}
